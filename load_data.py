...
class DataLoader:
    ...
    def load_graph(self, triplets, kg_triplets=None):
        ...
        adjs = torch.sparse_coo_tensor(indices=torch.LongTensor(edges).t(), values=torch.FloatTensor(values), size=torch.Size([self.all_ent, self.all_ent, all_rel+1]), requires_grad=False).to('cpu') #changed from .cuda()
        return adjs
